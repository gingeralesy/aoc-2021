...
(defun d15-a* (map width height start end)
  (let ((nodes (make-array (list height width) :initial-element NIL)))
    ...
    (labels ((node (x y) (aref nodes y x))
             ...
             (node< (a b)
               (let ((value-a (d15-node-f-value a))
                     (value-b (d15-node-f-value b))
                     (pos-a (d15-node-pos a))
                     (pos-b (d15-node-pos b)))
                 (cond
                   ((node= a b) NIL)
                   ((= value-a value-b)
                    (< (+ (car pos-a) (* width (cdr pos-a)))
                       (+ (car pos-b) (* width (cdr pos-b)))))
                   (T (< value-a value-b)))))
             (heuristic (from to) (+ (abs (- (car to) (car from))) (abs (- (cdr to) (cdr from)))))
             (neighbours (prev open)
