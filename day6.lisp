...
(defparameter *day6-input* (local-file #P"day6.txt" :error T))

(defun d6-data ()
  (with-open-file (stream *day6-input* :if-does-not-exist :error)
    (let ((scanner (cl-ppcre:create-scanner "\\d+"))
          (count 0)
          (values))
      (cl-ppcre:do-matches-as-strings (match scanner (read-line stream))
        (push (parse-integer match) values)
        (incf count))
      (values (nreverse values) count))))
...
model each fish as a single number that represents the number of days until it creates a new
lanternfish.
