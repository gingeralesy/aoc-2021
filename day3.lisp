...
(defparameter *day3-input* (local-file #P"day3.txt" :error T))

(defun d3-data ()
  "Reads the binary data and returns each value as part of a list. Also returns the count of data points and the number of bits used.
Note: This returns the values as numbers rather than strings, because it's more fun that way."
  (with-open-file (stream *day3-input* :if-does-not-exist :error)
    (loop with mask = #x1
          with bits = 0
          for line = (read-line stream NIL)
          while line
          for value = (parse-integer line :radix 2)
          when (< mask value)
          do (progn
               (setf mask (ash mask 1))
               (incf bits))
          collect value into values
          count value into count
          finally (return (values values count bits)))))
...
(defun d3p1 ()
  "Note: We do this with bitwise operations because they're more fun than strings."
  (multiple-value-bind (values size bits)
      (d3-data)
    (let* ((counters (make-array bits :element-type 'integer
                                      :initial-contents (make-list bits :initial-element 0)))
           (max (ash 1 bits))
           (mask (1- max))
           (gamma 0))
      (loop for value in values
            do (dotimes (n bits)
                 (when (< 0 (logand value (ash 1 n)))
                   (incf (aref counters n)))))
      (dotimes (n bits)
        (when (<= (/ size 2) (aref counters n))
          (incf gamma (ash 1 n))))
      (let ((epsilon (logand (lognot gamma) mask)))
        (values (* gamma epsilon) gamma epsilon counters)))))
...
- Start again with all 12 numbers and consider only the first bit of each number. There are fewer 0
  bits (5) than 1 bits (7), so keep only the 5 numbers with a 0 in the first position:
...
(defun d3p2 ()
  (multiple-value-bind (values size bits)
      (d3-data)
    (declare (ignore size))
    (flet ((common-values (values bit common)
             (loop with zeroes
                   with ones
                   for value in values
                   do (if (< 0 (logand value (ash 1 (- bits bit))))
                          (push value ones)
                          (push value zeroes))
                   finally (return (if (or (and common (< (length ones) (length zeroes)))
                                           (and (not common) (<= (length zeroes) (length ones))))
                                       zeroes
                                       ones)))))
      (let ((oxygen-rating
              (loop for oxygen = values then (common-values oxygen bit T)
                    counting oxygen into bit
                    while (and (<= bit bits) (rest oxygen))
                    finally (return (car oxygen))))
            (carbon-rating
              (loop for carbon = values then (common-values carbon bit NIL)
                    counting carbon into bit
                    while (and (<= bit bits) (rest carbon))
                    finally (return (car carbon)))))
        (values (* oxygen-rating carbon-rating) oxygen-rating carbon-rating)))))
